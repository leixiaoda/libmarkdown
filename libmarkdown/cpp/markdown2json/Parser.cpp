...
Sunset::Content Parser::loadFile(std::string file)
{
    ...
    // preprocess file
    preprocessFile(myReadFile, lines, YAMLLines);

    // Markdown processing
    processMarkdown(bodyModel, lines, 0, lines.size());
...
void Parser::processMarkdown(Sunset::BodyModel& bodyModel, std::vector<std::string> const &lines, int index, int length)
{
    if (index != length)
    {
        ...
        for (int i = 0; i < rules.size(); i++)
        {
            ...
            { // match
                ...
                {   // rule is multiline
                    ...
                    while (searchPointer + offset < lines.size() &&
                           //std::regex_match(lines.at(searchPointer + offset), rule.reg))
                           regex_match(lines.at(searchPointer + offset)) == rule.id)
                    {
                        par.text += " " + lines.at(searchPointer + offset);
                        offset++;
                    }
        ...
        processMarkdown(bodyModel, lines, index + 1 + offset, length);
    }
}
...
void Parser::processLine(Sunset::Paragraph &paragraph)
{
    ...
    case 10:
    case 12:
        if (paragraph.type == 9 || paragraph.type == 12) {
            removeIndex = 1;
        } else if (paragraph.type == 10) {
            // ordered list, removes "1. " prefix
            removeIndex = 2;
        } else {
...
//                            markup = {i, i + static_cast<int>(match[2].length()), 4};
                            markup = {convertIndex(paragraph.text, i), convertIndex(paragraph.text, i + static_cast<int>(match[2].length())), 4};
...
//                    markup = {i, 0, count};
                    markup = {convertIndex(paragraph.text, i), 0, count};
                    i--;
                    ...
                    if (!(stack.empty()) && (stack.top().type == markup.type)) {
                        markup = stack.top();
//                        markup.end = i+1;
                        markup.end = convertIndex(paragraph.text, i+1);
                        paragraph.markups.push_back(markup);
                        stack.pop();
                    } else {
                        stack.push(markup);
...
//                        markup = {i, static_cast<int>(i + match[1].length()), 3};
                        markup = {convertIndex(paragraph.text, i), convertIndex(paragraph.text, static_cast<int>(i + match[1].length())), 3};
                        markup.href = match[2];
        ...
        break;
    case 11:
        int pos = paragraph.text.find("](");
        if (pos != std::string::npos) {
            paragraph.url = paragraph.text.substr(pos + 2, paragraph.text.length() - pos - 3);
        }
        break;
    }
}
...
std::string Sunset::markdown_to_json(std::string markdown_str)
{
    ...
    std::stringstream ss(markdown_str);
    std::string item;
    std::string str(markdown_str);
    ...
    else
    {
        ...
        {
//            __android_log_print(ANDROID_LOG_DEBUG, "markdown2json", "%s", item.c_str());
